...
не будут удалены по завершении операции. Папка в любом случае очишается перед началом распаковки.
...
Предполагается, что все версии расширешия будут храниться в одном репозитории:
...
```
    ...
    > src
      > {код конфигурации}- папка содержит файлы актуальные для этой конфигурации 1С
        > {xyyzzz версия совместимости} - папка содержит файлы актуальные только для этой версии 1С
        build.xyyzz.cmd        - запускалка сборки версии
        extract.xyyzz.cmd      - запускалка разборки версии
        update_index.xyyzz.cmd - формирования и обновления индекса
        index.xyyzz.json       - словарь общих файлов
    build.cmd        - запускалка сборки всех расширений
    extract.cmd      - запускалка разборки всех расширений
```
При такой структуре содеижимое index.json будет симметричным для разных версий.
...
при таком подходе можно создать один пул процессов на всю сборку, это съэкономит чуток 
времени. 
