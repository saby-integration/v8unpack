...
В случаях когда код пишется для разных типов клиента есть необходимость для одного и 
того же кода в одном случае указывать директивы в другом случае не указывать.

Чтобы не дублировать код есть возможность автоматически устанавливать директивы внутри области include,
для этого достаточно к названию области добавить комментарий содержащий директиву.

Затем в содержимое области где эту директиву требуется установить добавить комментарий //DynamicDirective.
...
```
Заменит внутри себя все стоки начинающиеся с //DynamicDirective на &НаКлиенте//DynamicDirective
