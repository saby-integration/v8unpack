# 1.2
Выяснилось, что если установить разные расширения использующих одну и ту же обработку, то подмены одного
...
## 1.2.14
* fix - issues/31 - устранение замечаний по работе с контейнерами на низком уровне

## 1.2.13
* new - добавлен новый объект конфигурации WebSocketClient

## 1.2.12
* fix - при разборке обработки предназначенной для 8.2 но сохраненной в 8.3 не происходило раскомментирование директив 
        при разборке. Добавил использование параметра version для этих целей.
* fix - использование windows путей в секции index.json index.json на linux приводило к ошибке

## 1.2.11
* new - поддержка внешних отчётов (.erf) - добавлен тип ExternalReport в available_types и metadata_types (правка от
  MRDK80)
* fix - Большие значения Windows FILETIME в некоторых EPF-контейнерах вызывают OverflowError (правка от atlekbai)
* fix иногда не разбирались скобко файлы, если base64 параметр начинался не с новой строки, сделано допущение, что
весь прожолжающийся параметр это base64 строка

## 1.2.10
* fix на старых платформах 8.2 не работало переопределение панелей

## 1.2.9
* new параметр version работает одинаково для расширений и конфигураций - подменяет версию режима совместимости
  как при сборке так и разборке. убрана установка дефолтного значения для скомпилированных версий.
* new версия режима совместимости при сборке берется из сериализованного значения 'compatibility_version', раньше
  оно было справочно и не использовалось при сборке.
* Выяснили, что номер версии режима совместимости это всегда конкретное значение. Нельзя установить отсутствие
  режима совместимости. Это расчетное значение выводится когда текущая версия платформы 
  совпадает с версией режима совместимости продукта. 

* new параметр version может устанавливать версию режима совместимости и для конфигураций. 

## 1.2.8
* fix скомпилированная версия в случае ошибок не возвращала код выхода 1
 
## 1.2.7
* fix расширения у которых модуль приложения начинался с пустой строки не разбирались
