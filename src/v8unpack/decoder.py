...
class Decoder:
    ...
    @classmethod
    def encode(cls, src_dir, dest_dir, *, pool=None, version=None, release=None, file_name=None):
        helper.clear_dir(dest_dir)
        encoder = cls.get_encoder(src_dir, '803' if version is None else version[:3])
        tasks = encoder.encode(src_dir, dest_dir, version=version,
                               release=release, file_name=file_name
...
def encode(src_dir, dest_dir, *, pool=None, version='803', release=None, file_name=None):
    Decoder.encode(src_dir, dest_dir, pool=pool, version=version, release=release, file_name=file_name)
